...
if test_or_not==False:
    print('BlindMi_{0}_{1}.npz'.format(args.ndata, args.dataset))
    ...
    def diff_Mem_attack(x_, y_true, m_true, target_model, non_Mem_Generator=gen):
        ...
        for (mix_batch, m_true_batch) in S_target_prob_k:
            ...
            m_pred_dis_batch = np.ones(mix_batch.shape[0])
            m_pred_epoch = np.ones(mix_batch.shape[0])
            m_pred_dis_epoch = np.ones(mix_batch.shape[0])
            nonMemInMix = True
            while nonMemInMix:
                ...
                m_pred_dis_batch = m_pred_dis_epoch.copy()

            mix_shuffled.append(mix_batch)
            m_pred.append(m_pred_batch)
            m_pred_dis.append(m_pred_dis_batch)
            m_true.append(m_true_batch)
        return np.concatenate(m_true, axis=0), np.concatenate(m_pred, axis=0), \
               np.concatenate(mix_shuffled, axis=0), S_nonMem_prob_k, np.concatenate(m_pred_dis, axis=0)

    m_true, m_pred, mix, S_nonMem_prob_k, m_pred_dis = diff_Mem_attack(np.r_[x_train_tar, x_test_tar],
                                                  np.r_[y_train_tar, y_test_tar],
                                                  m_true, Target_Model)



    print(m_pred_dis.shape)
    print(m_pred_dis[:5])
    np.savez('BlindMi_{0}_{1}_sobel.npz'.format(args.ndata, args.dataset), m_pred=m_pred, m_true=m_true, m_pred_dis=m_pred_dis)
...
if test_or_not==True:
    ...
    npzdata = np.load('BlindMi_{0}_{1}_sobel.npz'.format(args.ndata, args.dataset))
    m_true = npzdata['m_true']
    m_pred = npzdata['m_pred']
    m_pred_dis = npzdata['m_pred_dis']

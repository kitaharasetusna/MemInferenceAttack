...
input_data = input_data_class_csv.InputDataCsv(dataset=dataset)
config = configparser.ConfigParser()
...
f_evaluate_logits = np.zeros([1,user_label_dim],dtype=float)
batch_predict=100
...
#############as we sort the prediction sscores,
# back_index is used to get back original scores#############
'''
Confused:
'''
...
'''
back_index:= every data in a record represents its order number
'''
...
origin_value_placeholder=tf.compat.v1.placeholder(tf.float32, shape=(1,user_label_dim)) #placeholder with original confidence score values (not logit)
label_mask = tf.compat.v1.placeholder(tf.float32, shape=(1,user_label_dim))  # one-hot encode that encodes the predicted label
...
for test_sample_id in np.arange(0,f_evaluate.shape[0]):
    ...
    while success==True: 
        ...
        while j<max_iteration \
                and (max_label!=result_max_label
                     or (result_predict_scores-0.5)*(result_predict_scores_initial-0.5)>0):
            gradient_values=sess.run(gradient_targetlabel,
                                     feed_dict={model.input:sample_f,
                                                origin_value_placeholder:origin_value,
                                                label_mask:label_mask_array,
                                                c3_placeholder:c3,
                                                c1_placeholder:c1,
                                                c2_placeholder:c2})[0][0]
...
np.savez(result_folder+"/attack/"+"noise_data_{}.npz".format(args.qt),
